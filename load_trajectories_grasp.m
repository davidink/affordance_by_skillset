function result = load_trajectories_grasp(dataFolder, scene_num, opt_save_kernel_feat, opt_save_feat_n_result)
    ...
    fig_gt = figure;
    step_size = 10;
    disp_trajectory_grasp(fig_gt, [dataFolder 'trajectory' num2str(scene_num)], step_size);
...
%         plotCoord(gripper_pose(1:3,4)',gripper_pose(1:3,1:3),0.025);
    ...
    %         plotCoord(cont_frame(1:3,4)',cont_frame(1:3,1:3),0.025);
            [bool_cont cont_pcd_EE cont_norm_EE cont_pcd_obj cont_norm_obj] = compute_contact_points_grasp(cur_gripper_points, cur_gripper_norms, cur_obj_modelpoints{i}, cur_obj_normalpoints{i}, cont_frame, dist_th, dot_th, num_th);        
            if bool_cont
