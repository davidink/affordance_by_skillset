function result = load_trajectories_kok_multiobj(dataFolder, scene_num, opt_save_kernel_feat, opt_save_feat_n_result)
    ...
    for time_step = 10:step_size:size(trajectory,1)-step_size % visualize according to the pose
        ...
        obj1_pose = [eGetR(obj_trajectory{1}(time_step,4:6)) obj_trajectory{1}(time_step,1:3)'; 0 0 0 1];
        nxt_obj1_pose = [eGetR(obj_trajectory{1}(time_step+step_size,4:6)) obj_trajectory{1}(time_step+step_size,1:3)'; 0 0 0 1];

        cur_action_pose = nxt_obj1_pose * obj1_pose^-1;
        ...
        for i=2:2
            ...
            cont_frame(4,4) = 1;
            ee_cent = mean(cur_obj_modelpoints{2});
    ...
    %         plotCoord(cont_frame(1:3,4)',cont_frame(1:3,1:3),0.025);
            [bool_cont cont_pcd_EE cont_norm_EE cont_pcd_obj cont_norm_obj] = compute_contact_points(cur_obj_modelpoints{1}, cur_obj_normalpoints{1}, cur_obj_modelpoints{i}, cur_obj_normalpoints{i}, cont_frame, dist_th, dot_th, num_th);        
            if bool_cont
                ...
                global_shape_features = compute_grid_feature(cur_obj_modelpoints{1}, cur_obj_modelpoints{i},cur_obj_pose_gf, 0.25, 0.05);
