function build_GP_models_mono(dataFolder)
    addpath('utilities');
    ...
    for item=1:2
        ...
        while ~bool_end 
            i = i+1;
            if(size(dir([dataFolder 'feat_n_result' num2str(item) num2str(i) '*']),1)==0)
                bool_end = true;
                scene_cnt = i -1;
            end
        end
        ...
        for i=1:scene_cnt
            filename = [dataFolder 'feat_n_result' num2str(item) num2str(i) '.mat'];
            load(filename);
            if item ==1
                results = result1;
            else
