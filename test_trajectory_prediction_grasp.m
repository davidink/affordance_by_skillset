function test_trajectory_prediction_grasp(dataFolder,scene_num)
    
    addpath('utilities');
    % display ground truth trajectory
    step_size = 10;
    fig_gt = figure;
    disp_trajectory_grasp(fig_gt, [dataFolder 'test_trajectory' num2str(scene_num)], step_size);
...
%     step_size = 20;
    features = [];
    results = [];
    load('data/models/GP_models_grasp.mat');
...
%         [gripper_l_points gripper_l_norms] = create_block_pcd(0.025,0.1,0.025,0.005,[-gripper_width/2 0.0 0.0]);
%         [gripper_r_points gripper_r_norms] = create_block_pcd(0.025,0.1,0.025,0.005,[gripper_width/2 0.0 0.0]);
        ...
        plotCoord(exp_gripper_pose(1:3,4)',exp_gripper_pose(1:3,1:3),0.025);
    ...
    %         plotCoord(cont_frame(1:3,4)',cont_frame(1:3,1:3),0.025);
            [bool_cont cont_pcd_EE cont_norm_EE cont_pcd_obj cont_norm_obj] = compute_contact_points_grasp(exp_gripper_points, exp_gripper_norms, cur_obj_modelpoints{i}, cur_obj_normalpoints{i}, cont_frame, dist_th, dot_th, num_th);        
            if bool_cont
                ...
                obj_pose_af = cont_frame * cur_obj_pose{i};
                pred_obj_pose_af = pred_obj_pose_diff * obj_pose_af;
                pred_obj_pose{i} = cont_frame^-1 * pred_obj_pose_af; 
