function result = load_trajectories_mono_step(dataFolder, scene_num, opt_save_kernel_feat, opt_save_feat_n_result)
    ...
    for time_step = 10:step_size:size(trajectory,1)-step_size % visualize according to the pose
        ...
        cur_obj1_pose_gf = [eGetR(obj_trajectory{1}(time_step,4:6)) obj_trajectory{1}(time_step,1:3)'; 0 0 0 1];
        cur_obj1_pose = [eGetR(obj_trajectory{1}(time_step,4:6)) obj_trajectory{1}(time_step,1:3)'; 0 0 0 1];
        cur_obj1_pose_gf(1:3,1:3) = cur_action_pose(1:3,1:3);
        cur_obj2_pose = [eGetR(obj_trajectory{2}(time_step,4:6)) obj_trajectory{2}(time_step,1:3)'; 0 0 0 1];

        %global_shape_features = compute_grid_feature(cur_gripper_points, cur_obj_modelpoints{i},cont_frame, 0.25, 0.05);
        global_shape_features = compute_grid_feature(cur_gripper_points, all_obj_modelpoints,cur_obj1_pose_gf, 0.25, 0.05);
        relative_pose_obj_to_obj = cur_obj1_pose*cont_frame^-1;
        relative_pose_features1 = [relative_pose_obj_to_obj(1:3,4)' RGete(relative_pose_obj_to_obj(1:3,1:3))'];
        relative_pose_obj_to_obj = cur_obj2_pose*cont_frame^-1;
        relative_pose_features2 = [relative_pose_obj_to_obj(1:3,4)' RGete(relative_pose_obj_to_obj(1:3,1:3))'];
        
        action_pose_features = [cur_action_pose(1:3,4)' RGete(cur_action_pose(1:3,1:3))'];

        all_features = [global_shape_features relative_pose_features1 relative_pose_features2 action_pose_features];

        if opt_save_feat_n_result
            obj1_pose_af = cur_obj1_pose*cont_frame^-1;
            nxt_obj1_pose = [eGetR(obj_trajectory{1}(time_step+step_size,4:6)) obj_trajectory{1}(time_step+step_size,1:3)'; 0 0 0 1];
            nxt_obj1_pose_af = nxt_obj1_pose*cont_frame^-1;
            obj1_diff = nxt_obj1_pose_af * obj1_pose_af^-1;
            
            obj2_pose_af = cur_obj2_pose*cont_frame^-1;
            nxt_obj2_pose = [eGetR(obj_trajectory{2}(time_step+step_size,4:6)) obj_trajectory{2}(time_step+step_size,1:3)'; 0 0 0 1];
            nxt_obj2_pose_af = nxt_obj2_pose*cont_frame^-1;
            obj2_diff = nxt_obj2_pose_af * obj2_pose_af^-1;
            ...
            cur_feat = all_features;
            cur_result1 = [obj1_diff(1:3,4)' RGete(obj1_diff(1:3,1:3))'];
            cur_result2 = [obj2_diff(1:3,4)' RGete(obj2_diff(1:3,1:3))'];

            features = [features;cur_feat];
            results1 = [results1;cur_result1];    
            results2 = [results2;cur_result2];
        end
       
    end

    if opt_save_feat_n_result
       save([dataFolder 'feat_n_result' num2str(scene_num) '.mat'],'features','results1','results2');
       disp('done!');
    end
    result = 1;
end
