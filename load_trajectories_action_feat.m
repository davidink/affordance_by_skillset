function result = load_trajectories_action_feat(dataFolder, scene_num, opt_save_kernel_feat, opt_save_feat_n_result)
    ...
    %         plotCoord(cont_frame(1:3,4)',cont_frame(1:3,1:3),0.025);
            ...
            if bool_cont
                ...
                if opt_save_feat_n_result
                    ...
                    if time_step == size(trajectory,1) % if current step is the end 
                        nxt_obj_pose = obj_pose; % end of execution
                        dontsave = true;
                    else
                        for j=time_step+step_size:step_size:size(trajectory,1)
                            nxt_obj_pose = [eGetR(obj_trajectory{i}(j,4:6)) obj_trajectory{i}(j,1:3)'; 0 0 0 1];
                            ...
                            action_feat = nxt_action_pose * gripper_pose^-1;
                            cur_feat = [kernel_feat sqrt(sum(action_feat(1:3,4).^2))];
