function [bool_cont cont_pcd_EE cont_norm_EE cont_pcd_obj cont_norm_obj] = compute_contact_points(cur_gripper_points, cur_gripper_norms, cur_obj_modelpoints, cur_obj_normalpoints, cont_frame, dist_th, dot_th, num_th)
...
%                     idx_norm = [];
%                     [idx_norm(:,1) idx_norm(:,2)] = find(ee_norms_CF(idx_ee,:)*obj_norms_CF(idx_obj,:)' < dot_th);
...
%         voxel_occu_obj(:,:,i) = occu_obj;
    ...
    if size(cont_idx_ee,1) ~=0 && size(cont_idx_obj,1) ~=0
        bool_cont = true;
%         plot3(ee_points_CF(cont_idx_ee,1),ee_points_CF(cont_idx_ee,2),ee_points_CF(cont_idx_ee,3),'Color',[0 0 1],'Marker','o','Linestyle','none');
%         plot3(obj_points_CF(cont_idx_obj,1),obj_points_CF(cont_idx_obj,2),obj_points_CF(cont_idx_obj,3),'Color',[0 1 1],'Marker','o','Linestyle','none');
