function modular_prediction_push(dataFolder, scene_num)
    ...
    for i=1:num_obj
        test_contact_through_traj(Desired_EE_pose,gripper_points,gripper_norms,obj_modelpoints{i},obj_normpoints{i},obj_trajectory{i}(1,:));
    end
    ...
    %         plotCoord(cont_frame(1:3,4)',cont_frame(1:3,1:3),0.025);
            ...
            if bool_cont
                ...
                if time_step == size(trajectory,1)
                    nxt_step_gripper_pose = [eGetR(Desired_EE_pose(time_step,4:6)) Desired_EE_pose(time_step,1:3)'; 0 0 0 1];
                else
