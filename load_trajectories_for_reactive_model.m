function result = load_trajectories_for_reactive_model(dataFolder, scene_num, opt_save_kernel_feat, opt_save_feat_n_result)
    ...
    for time_step = 10:step_size:size(trajectory,1)-step_size% visualize according to the pose
        ...
        if time_step == size(trajectory,1)
            ...
            exp_gripper_pose = [eGetR(Desired_EE_pose(time_step+step_size,4:6)) Desired_EE_pose(time_step+step_size,1:3)'; 0 0 0 1];
        end
        % cur_action_pose = nxt_gripper_pose * gripper_pose^-1;
        app_action_pose = exp_gripper_pose * gripper_pose^-1;


        figure(fig_traj);
        plot3(cur_gripper_points(:,1),cur_gripper_points(:,2),cur_gripper_points(:,3),'Color',[1 0 0],'Marker','.','Linestyle','none');
        hold on;
    %     plotCoord(gripper_pose(1:3,4)',app_action_pose(1:3,1:3),0.025);
    %     plot3(exp_gripper_pose(1,4),exp_gripper_pose(2,4),exp_gripper_pose(3,4),'o','MarkerSize',20);
    %     plot3(gripper_pose(1,4),gripper_pose(2,4),gripper_pose(3,4),'o','MarkerSize',15);
    ...
    %     quiver3(gripper_pose(1,4),gripper_pose(2,4),gripper_pose(3,4),app_action_pose(1,4),app_action_pose(2,4),app_action_pose(3,4));
    %     plot3(nxt_gripper_pose(1,4),nxt_gripper_pose(2,4),nxt_gripper_pose(3,4),'x','MarkerSize',15);
...
%             plot3(cur_obj_modelpoints{i}(:,1),cur_obj_modelpoints{i}(:,2),cur_obj_modelpoints{i}(:,3),'Color',default_color +[color_grad color_grad color_grad],'Marker','.','Linestyle','none');
        ...
        for i=1:num_obj
            ...
            cont_frame(1:3,1:3) = app_action_pose(1:3,1:3);
    %         plotCoord(cont_frame(1:3,4)',cont_frame(1:3,1:3),0.025);
            ...
            if bool_cont
                ...
                %save features for building up kernel
                    save([dataFolder 'kernel' num2str(scene_num) '_' num2str(time_step) '.mat'],'global_shape_features','local_contact_shape_features','app_action_pose');
                end
                %save features for building up kernel
