...
for smp_cnt=3:size(features,1)
    ...
    small_sigma_squared = 0.0001; %esitmated variance of y distribution
...
figure;
subplot(3,1,1);
plot(avg_RMSE_al(:,1),'-r');
hold on
plot(avg_RMSE_rnd(:,1),'-b');
subplot(3,1,2);
plot(avg_RMSE_al(:,2),'-r');
hold on
plot(avg_RMSE_rnd(:,2),'-b');
subplot(3,1,3);
plot(avg_RMSE_al(:,3),'-r');
hold on
plot(avg_RMSE_rnd(:,3),'-b');
