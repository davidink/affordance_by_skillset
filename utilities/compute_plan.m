function plan_set = compute_plan(obj_start_pose, obs_pose, goal_state,table_dim)
...
gprMdl_a = gprModel_cnst;
gprMdl_b = gprModel_free;
gprMdl_init = gprModel_nogroup;
...
%     result_features = csvread([dataFolder,'computed_features.txt']);
%     features_cand = result_features(:,2:end);
%     rslt_pushable = result_features(:,1);
...
%         output_pose_tf = output_pose_tf *output_pose_fix_rot';
        ...
        inside_reachable_region_right = in_polyhedron(fv_upright_right, [push_cand(i,1:3); current_obj_pose(1:3)]);
        inside_reachable_region_left = in_polyhedron(fv_upright_left, [push_cand(i,1:3); current_obj_pose(1:3)]);
        %if push_cand(i+2,1) < 0.3 % x < 0.3
        if inside_reachable_region_right | inside_reachable_region_left == 1 % reachable by either gripper
            reachability_flag = true;
        else reachability_flag = false;
        end
        
        if reachability_flag == false % drop off the cand
        else
            ...
            %open_features_set(current_open_set_size+valid_cand_cnt,:) = features_cand(i,:);
