function test_trajectory_prediction_action_feat(dataFolder,scene_num)
    addpath('utilities');
    %scene_num = 9;
    % display ground truth trajectory
    fig_gt = figure;
    disp_trajectory(fig_gt,dataFolder,scene_num);
    ...
    %         plotCoord(cont_frame(1:3,4)',cont_frame(1:3,1:3),0.025);
            ...
            if bool_cont
                ...
                %should add action feat in order to compute
                
                nxt_step_cnt = 1;
                for j=time_step+step_size:step_size:size(trajectory,1)
                    nxt_action_pose = [eGetR(Executed_EE_pose(j,4:6)) Executed_EE_pose(j,1:3)'; 0 0 0 1];
                    nxt_gripper_action = nxt_action_pose * exp_gripper_pose^-1;
                    action_feat(nxt_step_cnt) = sqrt(sum(nxt_gripper_action(1:3,4).^2));
                    nxt_step_cnt = nxt_step_cnt +1;
                end
                
                for k=1:size(action_feat,2)                    
                    feat = [kernel_feat action_feat(k)];
                    for j=1:7
                        [pred(k,j) pred_sd(k,j)] = predict(gprMdl{j},feat); 
                    end
                    pred_obj_pose_diff = [vrrotvec2mat(pred(k,4:7)) pred(k,1:3)'; 0 0 0 1];            
...
%                 for j=1:7
%                     pred_train(:,j) = resubPredict(gprMdl{j});
%                 end
